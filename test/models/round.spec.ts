...
describe('round methods', () => {
  describe('start', () => {
    ...
    it('correctly prepares the deck and hands', () => {
      ...
      expect(round.deck.cards.length).to.equal(46);
    ...
    it('deals 3 unique cards to each of 2 users, removing those 6 cards from the deck', () => {
      round.startRound(); // round 1, 3 cards

      // Deck and Users have correct card counts
      expect(round.deck.cards.length).to.equal(40);        
      const userCount = game.players.length;
      ...
      // tslint:disable-next-line: no-unused-expression
      expect(_.intersectionWith(round.hands[0].hand, round.deck.cards, _.isEqual)).to.be.empty;
      // tslint:disable-next-line: no-unused-expression
      expect(_.intersectionWith(round.hands[1].hand, round.deck.cards, _.isEqual)).to.be.empty;
    });

    it('prompts the first user to play', () => {
      expect(round.nextUp).to.deep.equal(users[0]);
